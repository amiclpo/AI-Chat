...
export function AdminPanel({
  ...
  const diagnostics = useMemo(() => {
    ...
    return {
      ...
      ragStatus: vectorCount > 0 
        ? getTranslation(language, "PEER_ON") 
        : getTranslation(language, "ADMIN_STATUS_NOT_CONFIGURED"),
    };
  }, [records, vectorCount, language]);
  ...
  const auditLogs = useMemo(() => {
    const events = [
      { id: "1", date: new Date(Date.now() - 30000).toISOString(), type: "SUCCESS", msg: getTranslation(language, "ADMIN_HEARTBEAT_LOG") },
      { id: "2", date: new Date(Date.now() - 120000).toISOString(), type: "AUTH", msg: getTranslation(language, "ADMIN_SOCKET_LOG") },
      { id: "3", date: new Date(Date.now() - 600000).toISOString(), type: "VECTOR", msg: getTranslation(language, "ADMIN_STORAGE_VERIFIED_LOG", { count: String(vectorCount) }) },
    ];
    ...
    records.slice(-5).forEach((rec, idx) => {
      events.push({
        ...
        date: rec.timestamp,
        type: "INFERENCE",
        msg: getTranslation(language, "ADMIN_INFERENCE_LOG", {
          id: rec.id.substring(0, 6),
          modelId: rec.modelId,
          tokens: String(rec.totalTokens),
          cost: rec.estimatedCost.toFixed(5)
        })
      });
    });
    ...
    return (
      <div className="max-w-md mx-auto my-12 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-xl space-y-6">
        <div className="flex flex-col items-center text-center space-y-2">
          ...
          <h2 className="text-lg font-bold text-zinc-900 dark:text-zinc-50">{getTranslation(language, "ADMIN_LOGIN_TITLE")}</h2>
          <p className="text-xs text-zinc-500 dark:text-zinc-400">
            {getTranslation(language, "ADMIN_LOGIN_SUBTITLE")}
          </p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest">
              {getTranslation(language, "ADMIN_LOGIN_PASSCODE_LABEL")}
            </label>
            <div className="relative">
              <input
                ...
                placeholder={getTranslation(language, "ADMIN_LOGIN_PASSCODE_PLACEHOLDER")}
          ...
          >
            {getTranslation(language, "ADMIN_LOGIN_AUTHORIZE_BTN")}
          </button>
        </form>
        ...
        <div className="rounded-xl bg-amber-500/10 border border-amber-500/20 p-3.5 flex items-start gap-2.5">
          ...
          <div className="text-[11px] text-zinc-600 dark:text-zinc-300 leading-relaxed font-sans">
            <span className="font-bold">{getTranslation(language, "ADMIN_BYPASS_INSTRUCTION")}</span> {getTranslation(language, "ADMIN_BYPASS_DESC").replace("admin123", "")} <code className="bg-amber-500/15 text-amber-600 dark:text-amber-400 font-bold px-1.5 py-0.5 rounded">admin123</code>. Click below to quickly fill.
            <button
              onClick={handleQuickFill}
              className="mt-2 block text-[10.5px] text-indigo-500 hover:underline font-semibold cursor-pointer"
            >
              {getTranslation(language, "ADMIN_BYPASS_FILL_BTN")}
            </button>
          </div>
        </div>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-zinc-200 dark:border-zinc-800 pb-5">
        <div>
          <h2 className="text-xl font-semibold text-zinc-900 dark:text-zinc-50 flex items-center gap-2">
            ...
            {getTranslation(language, "ADMIN_CONTROL_DESK_TITLE")}
          </h2>
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            {getTranslation(language, "ADMIN_CONTROL_DESK_SUBTITLE")}
          </p>
        </div>
        <button
        ...
        >
          {getTranslation(language, "ADMIN_REVOKE_CREDENTIALS")}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-4 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-850 flex items-center justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-wider font-bold text-zinc-400">{getTranslation(language, "ADMIN_DIAG_VM_MEMORY")}</p>
        ...
        <div className="p-4 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-850 flex items-center justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-wider font-bold text-zinc-400">{getTranslation(language, "ADMIN_DIAG_RAG_STATUS")}</p>
        ...
        <div className="p-4 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-850 flex items-center justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-wider font-bold text-zinc-400">{getTranslation(language, "ADMIN_DIAG_VECTOR_SIZE")}</p>
            <h4 className="text-md font-extrabold font-mono text-indigo-500 mt-1">{getTranslation(language, "ADMIN_INDICES_STORED", { count: String(vectorCount) })}</h4>
          </div>
          <div className="p-2 bg-zinc-50 dark:bg-zinc-800/60 rounded-lg text-indigo-500">
        ...
        <div className="p-4 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-850 flex items-center justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-wider font-bold text-zinc-400">{getTranslation(language, "ADMIN_DIAG_AVG_LATENCY")}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="p-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-850 rounded-xl space-y-4">
          <h3 className="text-sm font-semibold text-zinc-850 dark:text-zinc-250 flex items-center gap-2">
            ...
            {getTranslation(language, "ADMIN_PROXY_REGISTRY")}
          </h3>
            ...
            <div className="flex items-center justify-between p-2.5 rounded-lg bg-zinc-50 dark:bg-zinc-950 border border-zinc-100 dark:border-zinc-850">
              ...
              {geminiApiKey ? (
                <span className="text-[10px] bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                  <CheckCircle className="w-3 h-3" /> {getTranslation(language, "ADMIN_STATUS_ACTIVE_SECURE")}
                </span>
              ) : (
                <span className="text-[10px] bg-amber-500/10 text-amber-600 dark:text-amber-400 font-semibold px-2 py-0.5 rounded-full flex items-center gap-1">
                  <AlertTriangle className="w-3 h-3" /> {getTranslation(language, "ADMIN_STATUS_PLACEHOLDER")}
                </span>
              )}
            </div>
          ...
          <div className="pt-2 border-t border-zinc-150 dark:border-zinc-850 space-y-2">
            <h4 className="text-[10px] uppercase font-bold text-red-500">{getTranslation(language, "ADMIN_DESTRUCTIVE_ZONE")}</h4>
            <div className="flex flex-col gap-2">
              ...
              >
                ...
                {getTranslation(language, "ADMIN_CLEAR_TELEMETRY")}
              </button>
              <button
                type="button"
              ...
              >
                ...
                {getTranslation(language, "ADMIN_PURGE_RAG")}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 p-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-850 rounded-xl flex flex-col justify-between">
          <div className="space-y-1 pb-3">
            <h3 className="text-sm font-semibold text-zinc-850 dark:text-zinc-250 flex items-center gap-2">
              ...
              {getTranslation(language, "ADMIN_TOKEN_DIST_TITLE")}
            </h3>
            <p className="text-[11px] text-zinc-500 leading-none">
              {getTranslation(language, "ADMIN_TOKEN_DIST_SUBTITLE")}
            </p>
          </div>
          ...
          <div className="flex-1 w-full min-h-[190px] flex items-center justify-center">
            {pieChartData.length === 0 ? (
              <div className="text-xs text-zinc-400 font-sans text-center py-5">
                {getTranslation(language, "ADMIN_TOKEN_DIST_EMPTY")}
              </div>
            ) : (
      ...
      <div className="p-5 bg-zinc-950 border border-zinc-850 rounded-xl space-y-3">
        <h3 className="text-sm font-semibold text-zinc-100 flex items-center gap-2">
          ...
          {getTranslation(language, "ADMIN_DIAG_LOGS_TITLE")}
        </h3>
